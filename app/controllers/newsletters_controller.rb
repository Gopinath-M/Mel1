class NewslettersController < ApplicationController
  ...
  def create
    if params[:template][:type]=="All Users"
      temp_newsletter_to = "All Users"
      users = User.active
    elsif params[:template][:type]=="All Dept Admins"
      temp_newsletter_to = "All Dept Admins"
    ...
    if users && !users.empty?
      if temp_newsletter_to.kind_of?(Array)
        temp_newsletter_to.uniq!
        newsletter_to = temp_newsletter_to.join(", ").to_s
      else
        newsletter_to = temp_newsletter_to
      end
     p @newsletter = Newsletter.create!(params[:newsletter].merge!({:created_by => current_user.id,:from=>current_user.email, :status => "New",:to=> newsletter_to}))
      if @newsletter.valid?
        @newsletter.save!
