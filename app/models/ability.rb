class Ability
  ...
  def initialize(user)
    ...
    if user.is_super_admin?
      ...
      @approver_first = Approver.active.find_all_by_user_id_and_department_id(user.id,@current_department).first
      @approver_second = Approver.active.find_all_by_user_id_and_department_id(user.id,@current_department).last

      if ( @approver_first.present? || @approver_second.present? )
        can [:read,:create,:show,:approve_request,:request_view], ResourceTransportationBooking       
        can [:update,:list_ict_firewall],IctFirewall
      end
